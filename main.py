...
@app.get(
    ...
    description=(
        ...
        "value as 'timing unknown', never as 'no risk'. On the on-chain path it also reports "
        "vesting_deposit_amount/vesting_withdrawn_amount (Sablier's own depositAmount/"
        "withdrawnAmount, aggregated across streams) and vesting_progress_pct (withdrawn/deposit "
        "* 100), showing how far along the vesting schedule already is. No input parameters. "
        "Do not treat the "
...
@app.get(
    ...
    description=(
        ...
        "ownership renouncement, holder count, individual GoPlus risk signals "
        "(cannot_buy, cannot_sell_all, hidden_owner, transfer_pausable, selfdestruct, "
        "is_blacklisted, slippage_modifiable, owner_percent - these feed risk_flags), "
        "is_proxy/trading_cooldown (informational only, not flagged), and a summarized "
        "risk_level (LOW/MEDIUM/HIGH/UNKNOWN) with risk_flags. All GoPlus-only fields "
        "are null on the Honeypot.is fallback path. Data source is GoPlus Security "
...
@app.get(
    ...
    description=(
        ...
        "max_leverage, unrealized PnL, return_on_equity_pct, liquidation price, and "
        "distance-to-liquidation percentage (max_leverage and return_on_equity_pct are "
        "Hyperliquid's own reported fields, not derived by this endpoint). "
...
@app.get(
    ...
    description=(
        ...
        "separate forecast field). Also returns mark_price and index_price (populated on "
        "both the Bybit and Binance paths) and open_interest_usd (Bybit path only - "
        "always null on the Binance fallback, since Binance's premiumIndex endpoint does "
        "not report open interest). Input: required `symbol` query parameter (e.g. BTC, "
...
@app.get(
    ...
    description=(
        ...
        "of the two is required. net_spread_pct does NOT subtract the DEX pool's own "
        ...
        "that clears the threshold before those costs may not clear it after, so always "
...
@app.get(
    ...
    description=(
        ...
        "not just a top-of-book mirage - though `*_capacity_notional_usd` still prices "
        "that size at top-of-book (optimistic beyond the first price level); use "
        "`*_capacity_vwap_notional_usd` for the realistic average fill cost. Also returns "
        "`oldest_book_snapshot_time`, the staleness bottleneck across all legs (the oldest "
        "of each leg's own order-book snapshot timestamp). Input: "
        "required `event_slug` (from the event's URL "
...
@app.get(
    ...
    description=(
        ...
        "liquidity. Also returns `book_snapshot_time` (the book's own reported snapshot "
        "timestamp) and Polymarket's own `tick_size`/`min_order_size` for this market (null "
        "if the book response didn't include them). Input: required `position_size_shares`, "
        "and either `token_id` (if already "
